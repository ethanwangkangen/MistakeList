...
- LC 2528: **[RETRACTED — this was Claude's error, not Ethan's]** Claude claimed the diff array must be consumed at `adjustments[i]` rather than `adjustments[leftBound]`. A 200k-case stress test proved the opposite: consume-at-`leftBound` is CORRECT (0 fails), consume-at-`i` is WRONG (40k fails). Reason: window for city i is `[i-r, i+r]`; a station placed at slot `i+r` (to cover deficient city i) leaves city j's window when `j-r > i+r` ⇒ `j = i+2r+1`. Cancelling at `i+r+1` and consuming at `leftBound = j-r` fire the cancel exactly at `j = i+2r+1` — a matched pair. Consuming at `i` cancels r cities too early. **Lesson (Claude's): when unsure about an index-alignment claim, STRESS TEST against brute force before asserting — do not trace one lucky example and double down.**
...
- **LC 2528** (07-03) — Maximize the Minimum Powered City. BSoA correctly self-identified (monotonic feasibility). **Technique learned:** difference array for O(1) range-update during the greedy `feasible(x)` sweep — was the missing piece (didn't know it). Greedy placement = push forced stations as far right as still covers the deficient city, covering `[i, i+2r]`; consume at `leftBound` and cancel at `i+r+1` (matched pair — window-relative, not index-relative). **Real bugs (Ethan's):** `vector<int> adjustments` truncating long long deltas [G]; `int minimumPower` param truncating long long `mid` at the call boundary [G — narrowing-across-function-boundary]. Both would fail on large inputs (k≤1e9, power≤1e10). Countermeasure installed → §2.3 "type-width pre-pass." (Separately: two *other* things Claude flagged as bugs — consume-at-leftBound and `-=` — were NOT bugs; Claude retracted those after a 200k stress test. See §2.1-E.)
...
*Last updated: 2026-07-03 (LC 3977, 2528 + diff-array debugging appended) · Sessions mined: ~25 · Logged execution bugs: ~58 · Logged identification events: 15 · Techniques logged: difference array, BSoA ritual*
